...
public class Async {
	...
	static class ExecRunnable<T> implements Runnable {
		...
		ExecRunnable(final BasicFuture<T> future, final Request request, final Executor executor,
	...
	public <T> Future<T> execute(final Request request, final ResponseHandler<T> handler,
		...
		final ExecRunnable<T> runnable = new ExecRunnable<T>(future, request, this.executor != null ? this.executor
				: Executor.newInstance(), handler);
