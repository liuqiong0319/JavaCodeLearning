package com.vip.qa.autov.core.json.parser;
 ...
 * limitations under the Licimport static net.minidev.json.parser.ParseException.ERROR_UNEXPECTED_CHAR;
...
abstract class JSONParserStream extends JSONParserBase {
	...
	protected Object readNumber(boolean[] stop) throws ParseException, IOException {
		...
		if (c != '.' && c != 'E' && c != 'e') {
			...
			if (c >= 0 && c < MAX_STOP && !stop[c] && c != EOI) {
				...
				xs = sb.toString().trim();
				if (!acceptNonQuote)
					throw new ParseException(pos, ParseException.ERROR_UNEXPECTED_TOKEN, xs);
				return xs;
			}
			xs = sb.toString().trim();
	...
	protected void readString() throws ParseException, IOException {
		if (!acceptSimpleQuote && c == '\'') {
			...
			throw new ParseException(pos, ParseException.ERROR_UNEXPECTED_CHAR, c);
		}
		sb.clear();
		//
		//
